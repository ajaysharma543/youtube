...
import { resetSubscriptionState, setSubscriptionState, toggleSubscriptionFailure, toggleSubscriptions } from "../../redux/features/subscription";

function Subscriber({ video }) {
  const { data: user } = useSelector((state) => state.user);
  const {isSubscribed, loading,error} = useSelector((state) =>state.subscriber)
...
dispatch(setSubscriptionState(subscribed))
      ...
      console.log("Subscription failed:", error.response?.data || error.message);
